...
exports.logToFile = function(str, file){
    if(!file){ file = 'debug.log' }
        fs.appendFile(file, dateFormat(new Date(), 'yyyy-mm-dd HH:MM:ss') + '\t' + str + '\n', (err)=>{
            if(err) throw err;
    });
};
...
exports.reverseHex = function(hex){
    return exports.reverseBuffer(new Buffer(hex.toString(), 'hex')).toString('hex');
};
